...
namespace ShopCsharp.API
{
    ...
    public class WebServiceManager<T> where T : class
    {
        ...
        public async Task<Int32> Post(Int32 item)
        {
            Int32 result = default(Int32);
            String url = typeof(T).Name + "/";
            result = await HttpClientSender<Int32>(url, item, result);
        ...
        private async Task<TItem> HttpClientSender<TItem>(String url, TItem item,
TItem result)
        {
            using (HttpClient client = new HttpClient())
            {
        ...
        public async Task<Int32> Put(Int32 item)
        {
            Int32 result = default(Int32);
            String url = typeof(T).Name + "/";
            result = await HttpClientPuter<Int32>(url, item, result);
        ...
        private async Task<TItem> HttpClientPuter<TItem>
(string url, TItem item, TItem result)
        {
            using (HttpClient client = new HttpClient())
            {
                client.BaseAddress = new Uri(DataConnectionResource);
        ...
        private async Task<TResult> HttpClientDeleter<TItem, TResult>
(string url, TItem item, TResult result)
        {
            using (HttpClient client = new HttpClient())
            {
                client.BaseAddress = new Uri(DataConnectionResource);
